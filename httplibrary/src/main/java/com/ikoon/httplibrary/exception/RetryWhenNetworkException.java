...
public class RetryWhenNetworkException implements Function<Observable<? extends Throwable>, Observable<?>>
{
    ...
    public Observable<?> apply(Observable<? extends Throwable> observable) throws Exception
    {
        ...
        }).flatMap(new Function<Wrapper, ObservableSource<?>>()
        {
            ...
            {
                if ((wrapper.throwable instanceof ConnectException || wrapper.throwable instanceof SocketTimeoutException || wrapper.throwable instanceof TimeoutException) && wrapper.index < count + 1)
                {
...
//                            { //如果超出重试次数也抛出错误，否则默认是会进入onCompleted
//                                Log.e("tag", "retry---->" + wrapper.index);
//                                return Observable.timer(delay + (wrapper.index - 1) *
//                                        increaseDelay, TimeUnit.MILLISECONDS);
//                            }
//                        return Observable.error(wrapper.throwable);
            }
        });
